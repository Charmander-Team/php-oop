...
foreach ($pokemonModel->getList() as $value){ ?>
    <tr>
        ...
        <td><a href="origineindex.php?action=voir&id=<?php echo $value->getId(); ?>"><?php echo $value->getName(); ?></a></td>
        ...
        <td><a href="origineindex.php?action=update&id=<?php echo $value->getId(); ?>">Update</a></td>
        <td><a href="origineindex.php?action=delete&id=<?php echo $value->getId(); ?>">Delete</a></td>
    </tr>
<?php } ?>
    </table>
    <table border="1">
        ...
        if(!empty($_GET) && $_GET["action"]==="voir"){
            foreach ($pokemonModel->getOne($_GET["id"]) as $value){ ?>
                <tr>
                    <td><?php echo $value->getId(); ?></td>
                    <td><a href="origineindex.php?id=<?php echo $value->getId(); ?>"><?php echo $value->getName(); ?></a></td>
                    <td><?php echo $value->getLocation(); ?></td>
                    <td><?php echo $value->getType(); ?></td>
                    <td><?php echo $value->getHp(); ?></td>
                    <td><?php echo $value->getHasEvolve(); ?></td>
